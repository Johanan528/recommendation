...
class DCN(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, cate_feature_size, field_size,numeric_feature_size,
        ...
        assert loss_type in ["logloss", "mse"], \
            ...
            #对于分类任务用log loss 而 对于回归任务用mse评估函数，
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            # model
            self.embeddings = tf.nn.embedding_lookup(self.weights['feature_embeddings'],self.feat_index) # N * F * K  就是说，现根据有多少个种类 算出普通的embedding
            feat_value = tf.reshape(self.feat_value,shape=[-1,self.field_size,1])
            self.embeddings = tf.multiply(self.embeddings,feat_value)  #再把这里的value和embedi乘以下。

            self.x0 = tf.concat([self.numeric_value,
                                 tf.reshape(self.embeddings,shape=[-1,self.field_size * self.embedding_size])]
                                ,axis=1) #这是把他娘的（？，30，8）的embeddings和全他娘的弄一行去了。还和这个numeric——value拼一下子。


            # deep part                     deep part和 cross net都是以（？，规定的数）这种行列的格式输出到最后求权值的层。


            self.y_deep = tf.nn.dropout(self.x0,self.dropout_keep_deep[0])
#x0是输入，而self.dropout_keeo_keep[0]=0.5是指被放弃的概率。
            ...
            for l in range(self.cross_layer_num):
                x_l = tf.tensordot(tf.matmul(self._x0, x_l, transpose_b=True),  #注意这里用的点乘不是matmuply
                                    self.weights["cross_layer_%d" % l],1) + self.weights["cross_bias_%d" % l] + x_l
                #这里为什么是将x0，xl乘完以后的最后一维向量和 self.weights的第一列相乘？ 为什么用tensordot。
                #这里axis参数为1，即表示的就是内积。 这个tensordot好好理解理解。

            self.cross_network_out = tf.reshape(x_l, (-1, self.total_size))     #这里这个total_size挺有意思，他是filed数乘embedding_k+连续变量的种类。
            ...
            if self.l2_reg > 0:#l2_reg为外输入量 此时=0.0
                self.loss += tf.contrib.layers.l2_regularizer(
                    self.l2_reg)(self.weights["concat_projection"]) #加上L2正则
