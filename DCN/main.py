...
def load_data():
    ...
    def preprocess(df):
        ...
        df["missing_feat"] = np.sum((df[cols] == -1).values, axis=1) #这步为啥这么做 找到每行缺失的数据个数。
        df["ps_car_13_x_ps_reg_03"] = df["ps_car_13"] * df["ps_reg_03"]#还有这步
        return df
...
#分成三份，形成三次的train和test数据，并且S。。fold和kfold有不一样的地方，他的训练集中，是按照y取值的比例来分配。
