...
for col in df.columns:
    if col in IGNORE_COLS:
        ...
        feature_dict[col] = dict(zip(unique_val,range(total_feature,len(unique_val) + total_feature))) #可以将zip看作打包，用dict形成字典形式。
...
for col in train_feature_index.columns:
    if col in IGNORE_COLS:
        ...
        train_feature_index[col] = train_feature_index[col].map(feature_dict[col])#这句不是很明白。
...
feat_value = tf.placeholder(tf.float32, shape=[None, None], name='feat_value')  #可以理解预先分配内存。

label = tf.placeholder(tf.float32, shape=[None, 1], name='label') #一列的
...
weights['feature_embeddings'] = tf.Variable(
    tf.random_normal([dfm_params['feature_size'], dfm_params['embedding_size']], 0.0, 0.01),#均值0 标准差0.01
    name='feature_embeddings')
weights['feature_bias'] = tf.Variable(tf.random_normal([dfm_params['feature_size'], 1], 0.0, 1.0), name='feature_bias')
#初始化254个偏执量。
# deep layers
num_layer = len(dfm_params['deep_layers']) #what this？ 2
input_size = dfm_params['field_size'] * dfm_params['embedding_size']  #77 这个是什么尺度？
glorot = np.sqrt(2.0 / (input_size + dfm_params['deep_layers'][0]))#这斗神几把玩意 2/296+32 0.5次方
#是weight layer的标准差。
...
#这些参数大小还是很烦啊
# final concat projection layer 最后的合并投影层。

if dfm_params['use_fm'] and dfm_params['use_deep']:
    input_size = dfm_params['field_size'] + dfm_params['embedding_size'] + dfm_params['deep_layers'][-1]
elif dfm_params['use_fm']:
    input_size = dfm_params['field_size'] + dfm_params['embedding_size'] #fm的最后一层是37+8？？？
elif dfm_params['use_deep']:
    input_size = dfm_params['deep_layers'][-1] #deep是32
...
fm_first_order = tf.reduce_sum(tf.multiply(fm_first_order, reshaped_feat_value), 2) #你在这加你马呢
...
for i in range(0, len(dfm_params['deep_layers'])):
    y_deep = tf.add(tf.matmul(y_deep, weights["layer_%d" % i]), weights["bias_%d" % i]) #这里用的点积。
