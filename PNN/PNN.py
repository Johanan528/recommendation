...
class PNN(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            # Quardatic Singal
            quadratic_output = []
            if self.use_inner: #ipnn
                for i in range(self.deep_init_size):
                    theta = tf.multiply(self.embeddings,tf.reshape(self.weights['product-quadratic-inner'][i],(1,-1,1))) #这里用的是对应位置的乘积。# N * F * K
                    quadratic_output.append(tf.reshape(tf.norm(tf.reduce_sum(theta,axis=1),axis=1),shape=(-1,1))) # N * 1

            else:
                embedding_sum = tf.reduce_sum(self.embeddings,axis=1) #算的是f累加
                p = tf.matmul(tf.expand_dims(embedding_sum,2),tf.expand_dims(embedding_sum,1)) # N * K * K#f累加和f累加转置的矩阵乘。
                for i in range(self.deep_init_size):
            ...
            self.y_deep = tf.nn.relu(tf.add(tf.add(self.lz, self.lp), self.weights['product-bias'])) #这里这个add是指值叠加
